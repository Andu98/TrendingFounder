...
- Collected Data status actions now optimistically hide reviewed rows when `Show reviewed` is off, reuse the current snapshot for the first rerun, and persist the review status to Supabase in the background with dashboard caches cleared after the async write completes.
